...
# -*- coding:utf-8 -*-
# Author:pitmaner
# Datetime:2018/7/2 下午11:23
# Software: PyCharm
# Project: tushare_test
# Filename: getpriceinfo
...
def to_sql(data):
    # 根据自己的数据设置参数
    conn = MySQLdb.connect('localhost', "root", "root", "anjuke", charset="utf8")
    cursor = conn.cursor()
    # sql_create_database = "create database if not exists anjuke"
    # cursor.execute(sql_create_database)
    cursor.execute('''create table if not exists anjuke.xiaoqu(xiaoqu_id bigint(80),
    ...
    cursor.executemany(
        'insert ignore into anjuke.xiaoqu values(%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s);',
        data)
    # cursor.execute('select * from anjuke.xiqoqu limit 5;')
...
url = "https://m.anjuke.com/wh/xiaoqu-p"
